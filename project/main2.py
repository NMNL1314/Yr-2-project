...
if __name__ == "__main__":
    ...
    current_time = ""
    def display_time():
        global current_time, start_time, game_over
        clear_rect = pygame.Rect(800, 0, 200, 200)
        SCREEN.fill((255, 255, 255), clear_rect)

        if not game_over:
            current_time = (pygame.time.get_ticks() - start_time) // 1000

        time_surf = text_font.render(f'Time : {current_time}', False, (64, 64, 64))
        time_rect = time_surf.get_rect(center=(900, 100))
    ...
    def display_score():
        ...
        score_rect = score_surf.get_rect(center=(900, 150))
    ...
    while True:
        ...
        if game_result != "":
            play_again()
            if game_result == "W":  # Player won
                end_time = current_time
                # Calculate the score, subtracting a penalty for each incorrect guess
                penalty = (6 - guesses_count) * 5  # Adjust penalty as needed
                score = max(0, 500 - end_time + penalty + 100)
                game_over = True
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    ...
                    if key_pressed in "QWERTYUIOPASDFGHJKLZXCVBNM" and key_pressed != "" and not game_over:
                        game_active = True
